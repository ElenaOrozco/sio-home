...
class Archivo extends MY_Controller {
    ...
    public function arrayes(){
        $inputs = [
            0=>[
                "procesos"=>["Cedula", "nombres", "apellidos", "dni"],
                1=>["area"],
                2=>["1", "2"]

            ],
            1=>[
                0=>["MAnzana", "fresa", "durazno", "dni"],
                1=>["mucho", "400"],
                2=>["rrr", "2dff"]
        ...
        foreach ($inputs as $key => $value) {
            foreach ($inputs[$key] as $key2 => $value2) {
                foreach ($inputs[$key][$key2] as $key3=> $value3) {
                         echo $value3 ."<br>";
                }
            }
            # code...
        ...
        foreach ($inputs2 as $key => $value) {
            foreach ($inputs2[$key] as $key2 => $value2) {
                foreach ($inputs2[$key][$key2] as $key3=> $value3) {

                    foreach ($inputs2[$key][$key2][$key3] as $key4 => $value4) {
                        foreach ($inputs2[$key][$key2][$key3][$key4]  as $key5 => $value5) {
                        echo $value5 ."<br>";
                    }
                    }
    ...
    public function index() {
        ...
        $this->load->view('preregistro/v_pant_listado_obras.php', $data);
    ...
    public function historico_archivo($idArchivo){
        ...
        $this->load->model('archivo_model');
        $qProcesos = $this->archivo_model->get_estatus_fido($idArchivo);
    ...
    public function cambios($id_archivo = null, $idProceso = 1, $idSubProceso = 1, $idDocumento = 1, $Numero_Estimacion = "") {
        ...
        if ($id_archivo != 0) {
            ...
            $data["aArchivo"] = $this->archivo_model->datos_de_archivo($id_archivo)->row_array();
           ...
           $data["Estatus_doc"] = $this->load->datos_model->get_Estatus_relacion($idDocumento);
            ...
            /*$qProcesos= $this->archivo_model->procesos_de_archivo($id_archivo);
            $datos= array();
            $data_p = array();
            $data_Sub = array();
            ...
            foreach ($qProcesos->result() as $row) {
                $data_p['idUsuario_Trabajando'] = $row->idUsuario_Trabajando;
                $data_p['Proceso'] = $row->Proceso;
                $data_p['Estatus'] = $row->Estatus;
                //echo $row->Proceso;

                //Buscar los subprocesos del Proceso
                $qSubProcesos = $this->archivo_model->subprocesos_de_proceso($id_archivo, $row->idTipoProceso);
                
                foreach ($qSubProcesos->result() as $row_sub) {
                     
                    $data_SubProcesos = [
                        "SubProceso" => $row_sub->Nombre,
                        //"documentos" => $this->archivo_model->documentos_de_subproceso($id_archivo, $row_sub->id),
                    ];
                    $data_Sub[] =  $data_SubProcesos;
                   
                    //var_dump($data_SubProcesos);
                    //echo "<br><br>";

                    
                
                }
                $data_p['SubProcesos'] = $data_Sub;
                //var_dump($data_Sub);
                //echo "<br><br>";
            ...
            foreach ($datos as $value) {
                ...
                //print_r($value);
                echo $value['idUsuario_Trabajando'];
                echo "<h1>Proceso ". $value['Proceso'] . "<br></h1>";
                echo $value['Estatus'];
                //print_r( $value['SubProcesos']);
                
               
                foreach ($value['SubProcesos'] as  $sub) {
                    var_dump($sub);
                    echo '<h3><br>Nombre Subp:'. $sub['SubProceso']. "<br></h3>";
                    
                    /*foreach ($sub['documentos']->result() as $row) {
            ...
            $this->load->view('layouts/header', $data);
            $this->load->view('preregistro/v_pant_preregistro_enlaces', $data);
            $this->load->view('layouts/footer', $data); 
    ...
    public function llenar_plantilla(){
        ...
        $direccion_usuario = $this->session->userdata('idDireccion_responsable');
        $qProcesos = $this->archivo_model->procesos_de_archivo($idArchivo, $direccion_usuario);

        header('Cache-Control: no-cache, must-revalidate');
        header('Expires: Mon, 26 Jul 1997 05:00:00 GMT');
        header('Content-type: application/json');
        echo json_encode($qProcesos->result(), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
  
    }

    public function subprocesos_de_proceso(){
        $idArchivo = $this->input->post("idArchivo");
        $idTipoProceso = $this->input->post("idTipoProceso");

        $direccion_usuario = $this->session->userdata('idDireccion_responsable');
        $qSubProcesos = $this->archivo_model->subprocesos_de_proceso($idArchivo, $idTipoProceso, $direccion_usuario);
    ...
    public function documentos_de_subproceso(){
        $idArchivo = $this->input->post("idArchivo");
        $idTipoProceso = $this->input->post("idTipoProceso");
        $idSubProceso = $this->input->post("idSubProceso");

        $qDocumentos = $this->archivo_model->documentos_de_subproceso($idArchivo, $idTipoProceso, $idSubProceso);

        header('Cache-Control: no-cache, must-revalidate');
        header('Expires: Mon, 26 Jul 1997 05:00:00 GMT');
        header('Content-type: application/json');
        echo json_encode($qDocumentos->result(), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
    }

    public function documentos_de_archivo(){
        $idArchivo = $this->input->post("idArchivo");
       

        $qDocumentos = $this->archivo_model->documentos_de_archivo($idArchivo);
    ...
    public function traer_preregistrados(){

        $idArchivo = $this->input->post("idArchivo");

        $direccion_usuario = $this->session->userdata('idDireccion_responsable');
        $qPreregistrados = $this->archivo_model->traer_preregistrados($idArchivo, $direccion_usuario);

        header('Cache-Control: no-cache, must-revalidate');
        header('Expires: Mon, 26 Jul 1997 05:00:00 GMT');
        header('Content-type: application/json');
        echo json_encode($qPreregistrados->result(), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
    }

    public function traer_preregistro($idRAD, $direccion_usuario){
    ...
    public function actualizar_contadores($idArchivo, $idTipoProceso, $idSubProceso){
        $direccion_usuario = $this->session->userdata('idDireccion_responsable');
        $total_documentos_proceso = $this->archivo_model->total_documentos_proceso($idArchivo, $idTipoProceso);
        $total_documentos_subproceso = $this->archivo_model->total_documentos_subproceso($idArchivo, $idSubProceso);

        $preregistrados_proceso = $this->archivo_model->preregistrados_proceso_direccion($idArchivo, $idTipoProceso, $direccion_usuario);
        $preregistrados_subproceso = $this->archivo_model->preregistrados_subproceso_direccion($idArchivo, $idSubProceso, $direccion_usuario);
        $data = array(
            "proceso" =>$idTipoProceso,
            "subproceso" => $idSubProceso,
            "total_documentos_proceso"     => $total_documentos_proceso,
            "total_documentos_subproceso"  => $total_documentos_subproceso,
            "preregistrados_proceso"       => "Preregistrados: $preregistrados_proceso de $total_documentos_proceso",
            "preregistrados_subproceso"    => "Preregistrados: $preregistrados_subproceso de $total_documentos_subproceso"
        );
    ...
    public function preregistrar_documento(){
        $id                      = $this->input->post('id');
        $idArchivo               = $this->input->post('idArchivo');
        $tipo_documento          = $this->input->post('tipo_documento');
        $idTipoProceso           = $this->input->post('idTipoProceso');
        $idSubProceso            = $this->input->post('idSubTipoProceso');
        $direccion_usuario       = $this->session->userdata('idDireccion_responsable');
        $idUsuario               = $this->session->userdata('id');
        $tipoUsuario_preregistro = $this->session->userdata('preregistro');

        $data =  array();

        /* 1. Verificar si la direccion ya hizo preregistro del documento
        *     Si existe regresara el id 
        *  2. Si hay un id, Modificar Preregistro
        *  3. Si no hay id Insertar Preregistro
        */

        $preregistro                 = $this->traer_preregistro($id, $direccion_usuario);
        $idPreregistro               = $preregistro['id'];
        $data_retorno['preregistro'] = $preregistro['id'];

        //Si hay un preregistro de la direcciom
        if ($idPreregistro > 0){
            //Si el tipo de documento es 0 "Selecciona una opción"
            if ($tipo_documento == 0){
                $retorno = $this->archivo_model->eliminar_preregistro($idPreregistro, $data);
                $data_retorno = $this->actualizar_contadores($idArchivo, $idTipoProceso, $idSubProceso);
            } else {
                $data = array(
                    "tipo_documento"           => $tipo_documento,
                    "idUsuario_preregistra"    => $idUsuario,
                    "tipoUsuario_preregistro"  => $tipoUsuario_preregistro
                );
                $retorno = $this->archivo_model->modificar_preregistro($idPreregistro, $data);
            }
        } 
        //Si no hay un preregistro de la direcciom
        else {

            $data = array(
                "id_Rel_Archivo_Documento" => $id,
                "idArchivo"                => $idArchivo,
                "idDireccion_responsable"  => $direccion_usuario,
                "tipo_documento"           => $tipo_documento,
                "idUsuario_preregistra"    => $idUsuario,
                "tipoUsuario_preregistro"  => $tipoUsuario_preregistro,
                "fecha_preregistro"        => date('Y-m-d H:i:s'),
            );
            $retorno = $this->archivo_model->insertar_preregistro($data);
            if ($retorno > 0){
               $data_retorno = $this->actualizar_contadores($idArchivo, $idTipoProceso, $idSubProceso);
            }
        }
        ...
        $data_retorno['retorno'] = $retorno;
        echo json_encode($data_retorno);

    }

    public function preregistrar_hojas(){
        $noHojas           = $this->input->post('noHojas');
        $idRAD             = $this->input->post('idRAD');
        $direccion_usuario = $this->session->userdata('idDireccion_responsable');
        $data              = array();

        /* 1. Verificar si la direccion ya hizo preregistro 
        *  2. Si hay un id, Modificar Preregistro
        *  3. Si no mandar error
        */

        $preregistro                 = $this->traer_preregistro($idRAD, $direccion_usuario);
        $idPreregistro               = $preregistro['id'];
        $data_retorno['preregistro'] = $preregistro['id'];
        ...
        //Si no hay un preregistro de la direccion
        else {
            $retorno = -10;
    ...
    public function preregistrar($id_archivo = null, $idDireccion=null, $idProceso = 1, $idSubProceso = 1, $idDocumento = 1, $Numero_Estimacion = "") {
    ...
    public function preregistrar_documento_estimacion($id){
        ...
        $data["idDireccion_responsable"]=   $this->session->userdata('idDireccion_responsable');
        $data['fecha_preregistro']= date('Y-m-d H:i:s');
    ...
    //preregistro cid
    public function ingresar_estimacion_cid($id){
        $this->load->model('estimaciones_model');
        ...
        $data["idUsuario"]=  $this->session->userdata('id');
        $data["idDireccion_responsable"]=   $direccion;
        $data['tipoUsuario_preregistro']=1;
        $data['idUsuario_acepta']=$this->session->userdata('id');
        $data['preregistro_aceptado']=1;
        $data['fecha_preregistro']=date('Y-m-d H:i:s');
    ...
    public function cid_preregistra($idRelDoc_Archivo, $idArchivo, $idDireccion) {
        ...
        if ($this->input->post("preregistrado") == 0){
               ...
               $resultado = $this->rel_archivo_preregistro_model->datos_relacion_archivo_preregistro_delete($idRelDoc_Archivo, $idArchivo, $idDireccion);
        }else {
            //Regresa la relacion id, ...
            ...
            if($str_existe_preregistro->num_rows() > 0 )  {
                ...
                $datos['fecha_preregistro']=date('Y-m-d H:i:s');
                $datos['tipo_documento']= $this->input->post("preregistrado");
                $id = $str_existe_preregistro ->row_array();
                $resultado = $this->rel_archivo_preregistro_model->update_registro($datos, $id['id']);
                ...
                $datos=array();
                    ...
                    $datos['idUsuario_preregistra']=$this->session->userdata('id');
                    $datos['tipoUsuario_preregistro']=1;
                ...
                $resultado =   $this->rel_archivo_preregistro_model->datos_relacion_archivo_preregistro_insert($datos);
        ...
        $qSubTipoProceso_recibido=$this->rel_archivo_documento_model->listado_registros_preregistrados_por_sub_tipo_proceso_PRE_CID($aRegistro["idArchivo"],$aRegistro["idSubTipoProceso"]);
        
        $qTipoProceso_recibido=$this->rel_archivo_documento_model->listado_registros_preregistrados_por_tipo_proceso_PRE_CID($aRegistro["idArchivo"],$aRegistro["idTipoProceso"]);
    ...
    public function edit_preregistrados($idRelDoc_Archivo, $idArchivo) {
        ...
        if ($this->input->post("preregistrado") == 0){
            ...
            $this->rel_archivo_preregistro_model->datos_relacion_archivo_preregistro_delete($idRelDoc_Archivo, $idArchivo, $idDireccion);
    ...
    public function ver_estimaciones_relacion($idArchivo, $idRel){
     ...
     public function ver_estimaciones_PRE_CID($idArchivo, $idRel){
        $this->load->model('estimaciones_model');
        $this->load->model('subdocumentos_model');
        $this->load->model('direcciones_model');
        ...
        //json_encode($Direccion_responsable_documento);
        $responsable = $Direccion_responsable_documento['idDireccion_responsable'];
        $Direcciones = $this->direcciones_model->listado_catDirecciones();
                                    ...
                                    <div class="panel panel-default">
                                        ...
                                        <div id="collapse-'. $estimaciones->Numero_Estimacion  .'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading-'.$estimaciones->Numero_Estimacion  .'">
                                            <div class="panel-body">';
                                               ...
                                               if ($rEstimaciones->idDireccion_responsable > 0){ $d = "style = 'display:none' "; }else { $d = "  ";}               
   
                                               $div_estimaciones .='<div class="row">
                                                                        <div class="col-md-5">
                                                                            <h6> EST. '. $rEstimaciones->Numero_Estimacion .' - ' .$addw_SubDocumentos[$rEstimaciones->idSubDocumento] .'</h6>
                                                                            <div class="direccion-estimacion m-b">
                                                                               
                                                                                    <select class="form-control" name="DireccionEstimacion'.$rEstimaciones->id.'" '. $d .' id="DireccionEstimacion'.$rEstimaciones->id.'" >';
                                                                                        if($rEstimaciones->idDireccion_responsable== 0){


                                                                                            $div_estimaciones .='<option  value="0" selected="selected" >  SELECCIONA DIRECCIÓN </option>';
                                                                                        }


                                                                                        foreach ($Direcciones->result() as $rDireccion){

                                                                                            if($rEstimaciones->idDireccion_responsable >0 == $rDireccion->id){
                                                                                                $div_estimaciones .='<option   value ="'.$rDireccion->id .'" >'.$rDireccion->Nombre .'</option>';
                                                                                            }else{
                                                                                                $div_estimaciones .='<option   value ="'.$rDireccion->id .'"  >'.$rDireccion->Nombre .'</option>';
                                                                                            }
                                                                                        }
                                                                ...
                                                                $div_estimaciones .='</select>
                                                                        ...
                                                                        <div class="col-md-2">
                                                                            ...
                                                                            <select class="form-control m-b" name="tipo_documento_est'.$rEstimaciones->id .'" id="tipo_documento_est'.$rEstimaciones->id .'" onchange="ingresar_estimacion_cid('. $rEstimaciones->id .')" >
                                                                                <option value="'.$value_estimaciones .'" id="select" name="select">'. $seleccion_estimaciones .'</option>
...
//        $data["FechaTerminacion"] = $this->input->post("FechaTerminacion");
    ...
    /*public function agregar_observaciones_por_archivo($idArchivo) {
     ...
     public function agregar_observaciones_estimacion() {
        ...
        $retorno = $this->observaciones_estimacion_model->agregar_observacion_estimacion($data);
       ...
       /*
        ...
        header('Expires: Mon, 26 Jul 1997 05:00:00 GMT');
        header('Content-type: application/json');
        echo json_encode($retorn, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); 
    ...
    public function aceptar_preregistro($idArchivo, $direccion){
        $this->load->model('rel_archivo_documento_model');
        $this->load->model('rel_archivo_preregistro_model');
        $this->load->model('preregistro_model');
        $estimaciones= 0;
        ...
        $qSubDocumentos = $this->preregistro_model->get_subdocumentos($idArchivo, $direccion);
        
        if ($qSubDocumentos->num_rows() > 0){
            foreach ($qSubDocumentos->result() as $rSubDocumento) {
                $idRAD =  $rSubDocumento->idRel_Archivo_Documento;
            }
            $estimaciones = $this->preregistro_model->aceptar_estimaciones($idRAD, $direccion, $data);
        }
        
        
            
         //$this->rel_archivo_documento_model->update_rel_archivo_documento_aceptar_preregistro($idArchivo, $idDireccion_responsable, $data);
        $documentos  = $this->rel_archivo_preregistro_model->datos_relacion_archivo_preregistro_update_preregistro($data, $direccion, $idArchivo) ;
        if ($documentos >0 ){
        $strRetorno = "Aceptados $documentos Documentos";
        }
        if ($estimaciones >0 ){
            $strRetorno .= " <br> $estimaciones Documentos Estimaciones";
        }
        echo $strRetorno;
    ...
    public function actualizar_historial(){
        ...
        if (!$this->existe_historial($idArchivo, $Proceso, $idUsuario)){
            
            $data = array (
                'idArchivo' => $idArchivo,
                'idTipoProceso' => $Proceso,
                'idUsuario' => $idUsuario,
                'Motivo' => "CID Preregistra",
                'Fecha' =>  date('Y-m-d H:i:s'),
                'Estatus' => 10,
            );
            $retorno = $this->preregistro_model->agregar_historial($data);
            
            if ($retorno['retorno'] > 0 ) {return TRUE; } else {return FALSE ;}
        }
    ...
    public function existe_historial($idArchivo, $Proceso, $idUsuario){
        $this->load->model('preregistro_model');
        
        
        $existe =  $this->preregistro_model->retorna_historial($idArchivo, $Proceso, $idUsuario)->num_rows;
        
        if ($existe > 0 ) {return TRUE; } else {return FALSE ;}
