...
class Proyectos extends MY_Controller {
    ...
    public function eliminar_ubicacion($id, $cm_registro, $idUbicacion){
        ...
        $exito = $this->eliminar_registro($id, $cm_registro, $idUbicacion);
         
        if ($exito ==1){
            header('Location:'.site_url('proyectos/index/'));
        } else{
            header('Location:'.site_url('proyectos/index/' . "No_hubo_cambios"));
        }
    ...
    public function modificar_asignacion(){
        ...
        if ($cm > $cm_anteriores){
            //mover exito a si no cabe
            //Hacer eliminacion logica
            $exito = $this->eliminar_registro($id, $cm_anteriores, $idUbicacion);
        
            
            if ($exito){
                //cabe en el estante?
                $array_ubicacion = $this->proyectos_model->get_cm_utilizados($idUbicacion);
                $cabe = (($array_ubicacion['cm_utilizados']+ $cm) < 86 ) ? TRUE: FALSE;
                ...
                if (!$cabe){
                    ...
                    $actualizacion  = $this->ubicacion_proyectos_model->actualizar_cm($idUbicacion, $data);
                }
