...
class observaciones_estimacion_model extends CI_Model {
...
//        $registro = $this->db->insert_id();
     ...
     public function datos_observacion_estimacion_por_id($id) {
        $sql = 'SELECT * FROM `saaObservaciones_Estimaciones` WHERE id = ?';
    ...
    public function listado_observaciones_estimacion($idEstimacion) {
        $sql = 'SELECT  * 
                FROM `saaObservaciones_Estimaciones` 
                WHERE idEstimacion = ?
                        AND eliminacion_logica= 0
                        ORDER BY id DESC';
        $query = $this->db->query($sql, array($idEstimacion));
        return $query;
    }
    
    
    public function listado_observaciones_estimacion_direccion($idEstimacion, $direccion){
        $sql = 'SELECT  * 
                FROM `saaObservaciones_Estimaciones` 
                WHERE idEstimacion = ? AND idDireccion_responsable= ?
                        AND eliminacion_logica= 0
                        ORDER BY id DESC';
        $query = $this->db->query($sql, array($idEstimacion, $direccion));
        return $query;
    }
    ...
    public function listado_observaciones_estimacion_enlaces($idEstimacion) {
        $sql = 'SELECT  * 
                FROM `saaObservaciones_Estimaciones` 
                WHERE idEstimacion = ?
    ...
    public function listado_observaciones_estimacion_enlaces_vistos($idEstimacion){
        $sql = 'SELECT  * 
                FROM `saaObservaciones_Estimaciones` 
                WHERE idEstimacion = ?
    ...
    public function listado_observaciones_estimacion_cid($idEstimacion) {
        $sql = 'SELECT  * 
                FROM `saaObservaciones_Estimaciones` 
                WHERE idEstimacion = ?
    ...
    public function agregar_observacion_estimacion($data){
        
        
        $this->db->insert('saaObservaciones_Estimaciones',$data);
        ...
        if (!empty($registro)) {
                $this->log_new(array('Tabla' => 'saaObservaciones_Estimaciones', 'Data' => $data, 'id' => $registro));
        }
        if ($aff < 1) {
            if (empty($e)) {
                $e = "No se realizaron cambios";
    ...
    public function observaciones_estimacion_update($data, $id){
        
        $this->log_save(array('Tabla' => 'saaObservaciones_Estimaciones', 'Data' => $data, 'id' => $id));
        $this->db->update('saaObservaciones_Estimaciones', $data, array("id" => $id));
        $aff = $this->db->affected_rows();
