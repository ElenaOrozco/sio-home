...
class serie_model extends CI_Model {
    ...
    public function listado_serie() {
        //listar activos
        $sql = 'SELECT * FROM saaSerie WHERE Estatus=1 ORDER BY id ASC';
        $query = $this->db->query($sql);
        return $query; 
    }
    
     public function datos_serie($id) {
        $sql = 'SELECT * FROM saaSerie WHERE id = ?';
    ...
    public function datos_serie_insert($data) {
        $repetido =  $this->concepto_repetido(strtoupper($data['Nombre']));

        if( !$repetido['ret'] ){
            $this->db->insert('saaSerie', $data);
            ...
            if (!empty($registro)) {
                    $this->log_new(array('Tabla' => 'saaSerie', 'Data' => $data, 'id' => $registro));
            }
    ...
    public function datos_serie_update($data, $id) {
       
        $this->log_save(array('Tabla' => 'saaSerie', 'Data' => $data, 'id' => $id));
        
        $this->db->where('id', $id);
        $this->db->update('saaSerie', $data);
        $e = $this->db->_error_message();
        $aff = $this->db->affected_rows();
        $last_query = $this->db->last_query();
    ...
    public function concepto_repetido($str) {
        ...
        $query = $this->db->get_where('saaSerie');
        if ($query->num_rows() > 0) {
            $concepto = $query->row();
